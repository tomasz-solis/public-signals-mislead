...
def load_labeled_data(
    path: PathLike = "data/validation/labeled_features.csv",
...
def _calculate_cohens_d(supported_values: np.ndarray, pulled_back_values: np.ndarray) -> float:
    """Calculate Cohen's d using the average within-group variance."""
    ...
    if np.isnan(supported_std) or np.isnan(pulled_back_std):
        return np.nan

    pooled_std = np.sqrt((supported_std**2 + pulled_back_std**2) / 2)
    if pooled_std <= 0 or np.isnan(pooled_std):
        return np.nan
    return float((supported_summary["mean"] - pulled_back_summary["mean"]) / pooled_std)
...
def _decision_framework_prediction(row: pd.Series, majority_class: int) -> int:
    ...
    positive_signal = negative_ratio < POSITIVE_SIGNAL_NEGATIVE_RATIO_THRESHOLD
    negative_signal = negative_ratio > NEGATIVE_SIGNAL_NEGATIVE_RATIO_THRESHOLD
    high_mentions = total_mentions > HIGH_ENGAGEMENT_MENTION_THRESHOLD
...
def run_all_tests(labeled_path: PathLike = "data/validation/labeled_features.csv") -> Dict[str, object]:
...
def print_results(results: Dict[str, object]) -> None:
    ...
    print(
        ...
        f"{high_decay['threshold']:.0%} decay ({high_decay['pct_of_supported']:.0%})"
    )
...
def save_results(
    ...
    output_path: PathLike = "data/validation/statistical_results.csv",
) -> None:
